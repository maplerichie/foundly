...
import { publicLost } from "../helpers/polybase";
...
const LostsPage = ({ items, locations }: Props) => {
  ...
  const handleStatusChange = async (
    ...
    setStatus(status);
    const { data } = await publicLost.where("status", "==", status).get();
    let items = [];
    let locations = [];
          ...
          {itemList.length > 0 ? (
            <div className="grid grid-cols-3 gap-4">
              {itemList.map((item: any) => (
                <Card key={item.id} item={item} type={"lost"} />
              ))}
            </div>
          ) : (
...
LostsPage.getInitialProps = async ({}) => {
  const { data } = await publicLost.where("status", "==", 0).get();
  let items = [];
  let locations = [];
  for (let i = 0; i < data.length; i++) {
    items.push(data[i].data);
